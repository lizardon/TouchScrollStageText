/////////////////////////////////////////////////////////////////////////////////
// 
//	This file has been changed from its orgiginal by
//		Lizardon Software in 2013
//  The original verion of this file was:
//		spark.skins.mobile.supportClasses.StyleableStageText
//
////////////////////////////////////////////////////////////////////////////////
//
...
package com.lizardon.touchscrollstagetext
{
	 ...
	 *  @productversion Flex 4.6
	 */
	public class TouchScrollStyleableStageText extends UIComponent implements IEditableText, ISoftKeyboardHintClient
	{
		//--------------------------------------------------------------------------
		//
		//  Class variables
		 ...
		 *  @productversion Flex 4.6
		 */
		public function TouchScrollStyleableStageText(multiline:Boolean = false)
		{
			super();
		...
		private function updateWatchedAncestors():void
		{
			...
			for (i = 0; i < watchedAncestors.length; i++)
			{
				...
				if (newWatchedAncestors.indexOf(ancestor) == -1)
				{
					...
					ancestor.removeEventListener(TouchEvent.TOUCH_BEGIN, ancestor_onScrollerTouchBegin);
					ancestor.removeEventListener(TouchEvent.TOUCH_END, ancestor_onScrollerTouchEnd);
					if(ancestor is Scroller) Scroller(ancestor).viewport.removeEventListener(PropertyChangeEvent.PROPERTY_CHANGE, ancestor_onSrollerPropertyChange);
				}
			}
			...
			for (i = newWatchedAncestors.length - 1; i >= 0; i--)
			{
				...
				if (watchedAncestors.indexOf(newAncestor) == -1)
				{
					...
					if(newAncestor is Scroller)
					{
						newAncestor.addEventListener(TouchEvent.TOUCH_BEGIN, ancestor_onScrollerTouchBegin, false, 0, true);
						newAncestor.addEventListener(TouchEvent.TOUCH_END, ancestor_onScrollerTouchEnd, false, 0, true);
						Scroller(newAncestor).viewport.addEventListener(PropertyChangeEvent.PROPERTY_CHANGE, ancestor_onSrollerPropertyChange);
					}
				}
			}
		...
		protected var animationActive:Boolean = false;
		
		protected function ancestor_onScrollerTouchBegin(evt:TouchEvent):void
		{ 
			//trace("ancestor_onScrollerTouchBegin");
			beginAnimation();
		}
		
		protected function ancestor_onScrollerTouchEnd(evt:TouchEvent):void
		{ 
			//trace("ancestor_onScrollerTouchEnd");
			endAnimation();
		}
		
		private function ancestor_onSrollerPropertyChange(e:PropertyChangeEvent):void {
			if (e.source == e.target && e.property == "verticalScrollPosition")
			{
				if(!animationActive)
				{
					//trace("ancestor_onSrollerPropertyChange verticalScrollPosition");
					updateViewPort();
				}
			}
			if (e.source == e.target && e.property == "horizontalScrollPosition")
			{
				if(!animationActive)
				{
					//trace("ancestor_onSrollerPropertyChange horizontalScrollPosition");
					updateViewPort();
				}
			}
		}
...
import com.lizardon.touchscrollstagetext.TouchScrollStyleableStageText;
...
class StageTextPool
{
	 ...
	 */
	public static function acquireStageText(host:TouchScrollStyleableStageText):StageText
	{
	 ...
	 *  it released.
	 */
	public static function hasCachedStageText(host:TouchScrollStyleableStageText):Boolean
	{
	 ...
	 */
	public static function releaseStageText(host:TouchScrollStyleableStageText, stageText:StageText):void
	{
	...
	{
		var host:TouchScrollStyleableStageText = map_StageText_to_StyleableStageText[stageText];
